...
@Component({
  selector: 'app-hazri-list',
  templateUrl: './hazri-list.component.html',
  styleUrls: ['./hazri-list.component.scss']
})
export class HazriListComponent implements OnInit {
  ...
  deleteVoucher(voucher: Voucher): void {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      data: {
        ...
        message: `Are you sure you want to delete Hazri Voucher #${voucher.id}?`
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.voucherService.deleteVoucher(voucher.id).subscribe({
          next: () => {
            this.showSuccess('Hazri Voucher deleted successfully!');
            this.loadVouchers(); // Reload the list after deletion
          },
          error: (error) => {
            this.showError('Error deleting Hazri Voucher. Please try again.');
            console.error(error);
            this.loading = false;
          }
        });
