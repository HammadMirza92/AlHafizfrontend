...
@Component({
  selector: 'app-hazri-report',
  templateUrl: './hazri-report.component.html',
  styleUrls: ['./hazri-report.component.scss']
})
export class HazriReportComponent implements OnInit, AfterViewInit {
  ...
  initChart(): void {
    ...
    const ctx = document.getElementById('hazriChart') as HTMLCanvasElement;
    if (!ctx) return;

    // Group data by expense head
    const hazriData = this.dataSource.data.reduce((acc, voucher) => {
    ...
    const labels = Object.keys(hazriData);
    const data = Object.values(hazriData);
