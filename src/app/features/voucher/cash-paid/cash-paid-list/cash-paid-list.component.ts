...
@Component({
  selector: 'app-cash-paid-list',
  templateUrl: './cash-paid-list.component.html',
  styleUrls: ['./cash-paid-list.component.scss']
})
export class CashPaidListComponent implements OnInit {
  ...
  deleteTransaction(transaction: CashTransaction): void {
    const dialogRef = this.dialog.open(ConfirmationDialogComponent, {
      ...
      data: {
        ...
        message: `Are you sure you want to delete Cash Paid Voucher #${transaction.id}?`
      }
    });
    ...
    dialogRef.afterClosed().subscribe(result => {
      if (result) {
        ...
        this.cashTransactionService.deleteCashTransaction(transaction.id).subscribe({
          next: () => {
            this.showSuccess('Cash Paid Voucher deleted successfully!');
            this.loadTransactions(); // Reload the list after deletion
          },
          error: (error) => {
            this.showError('Error deleting Cash Paid Voucher. Please try again.');
            console.error(error);
            this.loading = false;
          }
        });
